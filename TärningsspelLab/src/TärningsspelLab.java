...
public class TärningsspelLab {
    public static void main(String[] args) {

        int userPlayer; // players for the game
        int computerPlayer;
        ...
        System.out.print("Enter the number for throws : ");

        int n=scan.nextInt();    // n is for number of times to roll the die
        int u1,u2,u3,u4,u5,c1,c2,c3,c4,c5;
        ...
        for (int turn = 1; turn <= n; turn++) {  //loop for continuous the process until our enterd no. of turns gets over
           ...
           // System.out.println();
            ...
            if (computerPlayer > userPlayer) {
                computerScore += 1;  //count for how many times win the computer player
                System.out.println("Computer Score: " + computerScore+"\tUser Score: " + userScore);

            } else if (userPlayer > computerPlayer) {
                userScore += 1;     //count for how many times win the user player
                System.out.println("Computer Score: " + computerScore+"\tUser Score: " + userScore);
